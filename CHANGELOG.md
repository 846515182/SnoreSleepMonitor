...
- 四类事件统计卡片**集成累计时长**（打鼾 / 磨牙 / 梦话 / 呼吸暂停各自显示次数 + 时长），
  删除下方独立的「各类事件累计时长」区块。
- 「实时音量」卡片头右侧直接显示当前分贝值，去掉重复的阈值徽章与第二行音量读数，
  「峰值 + 声音时长」合并为一行。

**设置页大幅缩短：**

- 四个阈值大区块（各带 3–4 行长文案 + 独立滑条）**集成**为一个「检测阈值」分段切换器：
  打鼾 / 磨牙 / 梦话 / 暂停四选一，只渲染当前类别的说明与滑条，说明文案精简。
- 原先 5 张独立卡片（阈值 / 应用更新 / 主题隐私 / 后台运行 / 存储空间）**合并为 2 张**：
  「检测阈值」+「通用设置」（更新、主题、隐私、后台、存储用分隔线分区）。
- 操作按钮改用紧凑样式（高度降低），各区块描述文案精简为一行。

### ✅ 验证

- `tsc --noEmit` 与 `tsc -p tsconfig.test.json` 均 0 错误；
- 单元测试 25/25 通过。
